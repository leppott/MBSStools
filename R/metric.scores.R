...
metric.scores <- function(DF_Metrics
  ...
  # @param Thresh Data frame of Scoring Thresholds (Index.Name
  # , Index.Region
  # , Metric
  # , Direction
  # , Thresh.Lo
  ...
  #   myMsg <- paste0("Fields missing from input data frame.
  ...
  for (a in unique(as.matrix(DF_Metrics[,IndexName]))){##FOR.a.START
    for (b in unique(as.matrix(DF_Metrics[,IndexRegion]))) {##FOR.b.START
      for (c in MetricNames){##FOR.c.START
        ...
        fun.Thresh.myMetric <- dplyr::filter(DF_Thresh
                                             , Index.Name==a &
                                               Index.Region==b &
                                               MetricName.Other==c)
        # QC
        ...
        if(fun.ScoreRegime=="100"){##IF.scoring.START
          if(fun.Direction=="Decrease"){
            fun.Result <- stats::median(c(0, 100, 100 * ((fun.Value-fun.Lo) /
                                                           (fun.Hi-fun.Lo))))
          }else if (fun.Direction=="Increase") {
            fun.Result <- stats::median(c(0, 100, 100 * ((fun.Hi-fun.Value) /
                                                           (fun.Hi-fun.Lo))))
          }
  ...
  if("TOTCNT" %in% names(DF_Metrics)){
    ...
    if(myTF_zero_n >0){
      msg_QC_NumOrg_Fish <- paste0("One or more fish samples (n = "
                                   , myTF_zero_n
                           , ") had zero organisms and was scored as a 1
                              (metrics and IBI).")
    ...
    msg_QC_NumOrg_Bugs <- paste0("One or more bug samples had < 60 (n = "
                                 , myTF_small_n
                                 ,") or > 120 (n = "
                                 , myTF_large_n
                                 ,") organisms.\n
  ...
  ## Fish
  myTF <- DF_Metrics[,IndexName] == "MBSS.2005.Fish" &
    DF_Metrics[, IndexRegion] == "COASTAL"
  DF_Metrics[, "IBI"][myTF] <- DF_Metrics[myTF, "sum_IBI"] / 6

  myTF <- DF_Metrics[,IndexName] == "MBSS.2005.Fish" &
    DF_Metrics[, IndexRegion] == "EPIEDMONT"
  DF_Metrics[, "IBI"][myTF] <- DF_Metrics[myTF, "sum_IBI"]/6

  myTF <- DF_Metrics[,IndexName] == "MBSS.2005.Fish" &
    DF_Metrics[, IndexRegion] == "HIGHLAND"
  DF_Metrics[, "IBI"][myTF] <- DF_Metrics[myTF, "sum_IBI"]/6

  myTF <- DF_Metrics[,IndexName] == "MBSS.2005.Fish" &
    DF_Metrics[, IndexRegion] == "COLD"
  DF_Metrics[, "IBI"][myTF] <- DF_Metrics[myTF, "sum_IBI"]/4

  ## Bugs, Genus
  myTF <- DF_Metrics[,IndexName] == "MBSS.2005.Bugs" &
    DF_Metrics[, IndexRegion] == "COASTAL"
  ...
  ## Bugs, Family
  myTF <- DF_Metrics[,IndexName] == "MSW.1999.Bugs" &
    DF_Metrics[, IndexRegion] == "CP"
  DF_Metrics[, "IBI"][myTF] <- DF_Metrics[myTF, "sum_IBI"]/7

  myTF <- DF_Metrics[,IndexName] == "MSW.1999.Bugs" &
    DF_Metrics[, IndexRegion] == "NCP"
  DF_Metrics[, "IBI"][myTF] <- DF_Metrics[myTF, "sum_IBI"]/7
