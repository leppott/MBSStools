...
#'
#' This function compares the user's data frame to a data frame with the official
#' (or user supplied) master taxa list (fish or benthic macroinvertebrates).
#'
...
#'
#' Any columns in the user input file that match the official master taxa list will
#' be renamed with the "_NonMBBS" suffix.
#'
#' New/different taxa in the user data are handled by the 'useMBSSTaxaInfo' parameter.
#' For taxa that did not match the master taxa list the user has options on how to
#' handle the differences for the phylogeny (e.g., columns for phylum, class,
#' family, etc.) and autecology (e.g., columns for FFG, habit, tolerance value
#' , etc.).  The options are below.
#'
#' * only_MBSS = use only MBSS master taxa information.  Any non-matching taxa will
#' not have any master taxa information.
#'
#' * only_user = only use the information provided by the user.  Information from the 'MBSS'
#' will not be used.  This should only be used for non-official calculations of the MBSS IBIs.
#'
#' * add_new = hybrid approach that uses MBSS master taxa information, when present, but includes user
#' information for non-matching taxa if the column names match.
#'
...
#'
#'     + TAXON, Phylum, Class, Order, Family, Genus, Other_Taxa, Tribe, FFG, FAM_TV
#'       , Habit, FinalTolVal07, Comment
#'
#' **Fish**
#'
#'     + SPECIES, TYPE, PTOLR, NATIVE, TROPHIC, SILT, PIRHALLA, DATA.ADDED, REASON
#'       , SOURCE, FAM, GENUS, SP_SCI, IN_KEY, APPROX_ID
#'
#' @param DF_User User taxa data.
#' @param DF_Official Official MBSS master taxa list.  Can be a local file or from a URL.
#' Default is NULL.  A NULL value will use the official online files.
#' @param fun.Community Community name for which to compare the master taxa list (bugs or fish).
...
#'
#' df_qc_taxa_fish <- qc_taxa(DF_User, DF_Official, fun.Community, useMBSSTaxaInfo)
#'
#' # QC input/output
#' dim(DF_User)
#' dim(df_qc_taxa_fish)
...
qc_taxa <- function(DF_User
                   , DF_Official = NULL
                   , fun.Community = NULL
                   , useMBSSTaxaInfo = "only_MBSS") {##FUNCTION ~ mastertaxa ~START
  #
  boo_DEBUG <- FALSE
  ...
  if (fun.Community == "bugs") {##IF.START
    ...
    col_mt <- c("Taxon", "Phylum", "Class", "Order", "Family", "Genus", "Other_Taxa"
                , "Tribe", "FFG", "FAM_TV", "Habit", "FinalTolVal07", "Comment")
    col_taxon <- col_mt[1]
  } else if(fun.Community == "fish"){
    url_mt <- "https://github.com/leppott/MBSStools_SupportFiles/raw/master/Data/CHAR_Fish.csv"
    col_mt <- c("SPECIES", "TYPE", "PTOLR", "NATIVE", "TROPHIC", "SILT", "PIRHALLA"
                ,"DATE.ADDED", "REASON", "SOURCE", "FAM", "GENUS", "SP_SCI", "IN_KEY"
                ,"APPROX_ID" )
    col_taxon <- col_mt[1]
  } else {
  ...
  if(sum_taxa_match != taxa_user_n){
    ...
    msg_1 <- paste0("The following user ", str_tax, " (", n_nonmatch, "/", taxa_user_n
  ...
  #names(DF_User)[boo_col_match] <- paste0(names(DF_User)[boo_col_match], "_NonMBSS")
  ...
  if(useMBSSTaxaInfo == "only_MBSS"){
    ...
    #   df_merge[df_merge[, col_taxon] == taxa_nonmatch, paste0(col_mod, "_NonMBSS")]

    df_result <- merge(DF_User, df_mt
                       , by = col_taxon
                       , all.x = TRUE
                       , suffixes = c("", sfx_NonMBSS))
    col_match_y <- names(df_result)[grepl(paste0(sfx_NonMBSS,"$"), names(df_result))]
    ...
    msg <- "Valid values for useMBSSTaxaInfo are 'only_MBSS', 'only_user', or 'add_new'."
    stop(msg)
  }
